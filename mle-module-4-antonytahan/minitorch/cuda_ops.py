...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    accum = 0
    # iterate over columns with block size as step
    for k_start in range(0, a_shape[2], BLOCK_DIM):
        k = k_start + pj
        # guard for a to ensure we are within bounds
        if i < a_shape[1] and k < a_shape[2]:
            # storing a value into the final position in shared memory
            a_shared[pi, pj] = a_storage[
                a_batch_stride * batch + a_strides[1] * i + a_strides[2] * k
            ]
        # guard for b to ensure we are within bounds
        if j < b_shape[2] and k < b_shape[1]:
            # storing b value into the final position in shared memory
            b_shared[pi, pj] = b_storage[
                b_batch_stride * batch + b_strides[1] * k + b_strides[2] * j
            ]
        # sync threads
        cuda.syncthreads()
        # iterate over block size to populate output
        for k in range(BLOCK_DIM):
            # ensure we are within bounds for a, b (a_shape[2] is the same as b_shape[1])
            if (k_start + k) < a_shape[2]:
                # increment value of res based on values in shared memory
                accum += a_shared[pi, k] * b_shared[k, pj]
    # fill in output storage with the result we have been incrementing
