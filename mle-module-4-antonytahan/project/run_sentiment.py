...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        if not self.training:
            # No dropout being done
            x_drop = minitorch.dropout(x_max, 0.25, True)
        else:
            # Dropout
            x_drop = minitorch.dropout(x_max, 0.25, False)

        x_lin = self.lin.forward(x_drop.view(x_drop.shape[0], x_drop.shape[1]))
        # x_lin = self.lin.forward(x_max.view(x_max.shape[0], x_max.shape[1]))
        # x_drop = minitorch.dropout(x_lin, 0.25)
        # return x_drop.sigmoid().view(x_drop.shape[0])
        return x_lin.sigmoid().view(x_lin.shape[0])
