...
class Network(minitorch.Module):
    ...
    def forward(self, x):
        ...
        x_avgpool = minitorch.avgpool2d(self.x2, (4, 4))
        x_avgpool = x_avgpool.view(BATCH, 392)
        x_avgpool = self.lin1.forward(x_avgpool).relu()

        # we should only be using dropout on training, not validation
        if not self.training:
            # No Dropout
            x_drop = minitorch.dropout(x_avgpool, 0.25, True)
        else:
            # Dropout
            x_drop = minitorch.dropout(x_avgpool, 0.25, False)
