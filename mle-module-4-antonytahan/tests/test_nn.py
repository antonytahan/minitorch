...
def test_max(t: Tensor) -> None:
    ...
    test_max = minitorch.max(t, 0)
    assert test_max[0, 0, 0] == max(t[i, 0, 0] for i in range(2))
    test_max = minitorch.max(t, 1)
    assert test_max[0, 0, 0] == max(t[0, i, 0] for i in range(3))
    test_max = minitorch.max(t, 2)
    assert test_max[0, 0, 0] == max(t[0, 0, i] for i in range(4))
    rand_tensor = minitorch.rand(t.shape)
    new_t = t + (rand_tensor * 1e-5)
    minitorch.grad_check(lambda t: minitorch.max(t, 2), new_t)
